...
describe('runUpdatePreflight', () => {
  ...
  it('global region: derives install commands and site links from the .ai profile', async () => {
    ...
    try {
      ...
      // region surface there is the manual install command text. The fork
      // publishes no install.ps1, so the win32 instruction is a download
      // link to the fork's release page, independent of region.
      expect(installCommandFor('native', '0.5.0', 'win32')).toBe(
        'download the latest win32 build from ' +
