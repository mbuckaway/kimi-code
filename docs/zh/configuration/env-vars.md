...
| `KIMI_CODE_EXPERIMENTAL_KIMI_OAUTH` | 在 web UI 和 TUI 中启用 kimi.com/kimi.ai 的 OAuth 设备码登录（默认关闭；token/apiKey 鉴权仍为默认）；master `KIMI_CODE_EXPERIMENTAL_FLAG=1` 也会启用本功能 | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
