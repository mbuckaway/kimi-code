...
| `/planning-model` | — | Set the model used while Plan mode is active: a model alias as an argument sets it directly, no argument opens the model picker, and `clear`/`unset` removes it (writes `planning_model`; see [Configuration Files](../configuration/config-files.md#top-level-fields)) | Yes |
