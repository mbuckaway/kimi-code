...
| `KIMI_CODE_EXPERIMENTAL_KIMI_OAUTH` | Enable kimi.com/kimi.ai OAuth device-code login for the web UI and TUI (disabled by default; token/apiKey auth remains the default); the master `KIMI_CODE_EXPERIMENTAL_FLAG=1` also enables it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
