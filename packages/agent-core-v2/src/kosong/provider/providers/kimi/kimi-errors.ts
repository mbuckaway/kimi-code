 ...
 * recharge your account …", and arrears phrasing); on a 403, the managed
 * subscription's usage-limit wording observed in
 ...
 * next cycle. …"). Every 429 pattern is anchored to billing wording —
 * deliberately no bare /quota/ or /balance/, so transient throttle messages
 * like "token quota per minute" keep classifying as retryable rate limits;
 * and a 403 is promoted only on the usage-limit-specific wordings, so an
 * ordinary auth/permission 403 keeps the base classification. The classifier
 * reads the raw SDK error structurally (status / code / type / message), so
 * it works over both the OpenAI and Anthropic transports: the OpenAI SDK
