...
describe('Agent loop', () => {
  ...
  it('fails the turn after a filtered step completes', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [wire] llm.request                 { "agentId": "main", "kind": "loop", "provider": "openai", "model": "mock-model", "modelAlias": "mock-model", "thinkingEffort": "off", "maxTokens": 1000000, "toolSelect": false, "systemPromptHash": "ec9c34379c88babbc468ef2f3e0e08cd2f422c8c4a910664fb8bb394d703a575", "toolsHash": "345824416b48dd70b3bf5ec60bf465b92abe64e452d9ad27d183f897615e0fe3", "messageCount": 1, "turnStep": "0.1", "time": "<time>" }
  ...
  it('streams partial content live but does not persist it when the request fails mid-stream', async () => {
    profile.update({ activeToolNames: [] });
    ctx.mockNextProviderResponse({
    ...
    expect(ctx.allEvents).toContainEqual(
      expect.objectContaining({
        ...
        args: expect.objectContaining({ delta: 'partial before error' }),
      }),
    );
